...
theory Bounded_Buffer_200924_ddl_automation_imporved
...
\<comment> \<open>This version uses fixrec to model Bounded Buffer
The verification  is through simulation strategy
the deadlock-freedom is verified, but the refinement is not proved.
two lemmas added: output_refine, input_refine by Simon
deadlock_free_BBuf proof is improved by using the two new lemmas input_refine and output_refine compared to 120924_ddf version. \<close>
...
\<comment> \<open>this proof below is improved by using the two new lemmas input_refine and output_refine compared to 120924_ddf version. \<close>
    ...
    next
      ...
      proof -
        ...
        proof cases
         ...
         (* 'apply (rule input_refine)'  above replace the following
            apply (simp add: read_def)
            apply (rule trans_FD[OF Mndetprefix_trans_subset_FD Mprefix_refines_Mndetprefix_FD])
            apply simp_all
            *)
...
*)
           apply simp
            ...
            apply (rule Ndet_trans_Det_FD)
             ...
             apply (rule input_refine)
 (*         line above replace the following 2 lines
