...
|
[simp del] :\<open>Trans_ex1'''\<cdot>n = 
    (((n = NID_i0_stm0) \<^bold>&  ((internal__stm0\<^bold>.NID_i0_stm0 \<rightarrow> Skip)\<^bold>; (enter_s0_stm0 \<rightarrow> Skip )))\<^bold>;Trans_ex1'''\<cdot>NID_s0_stm0)
	  \<box>
	 ( ((n = NID_s0_stm0) \<^bold>& (((a__in\<^bold>.NID_s0_stm0 \<rightarrow> Skip) \<^bold>;( SSTOP \<triangle> (exit_stm0 \<rightarrow> Skip))\<^bold>;  ( SSTOP \<triangle>((exited_stm0 \<rightarrow> Skip)\<^bold>; (enter_s1_stm0 \<rightarrow>  Skip ))))))\<^bold>; Trans_ex1'''\<cdot>NID_s1_stm0)
	  \<box>
	 ( ((n = NID_s1_stm0) \<^bold>& ((b__in\<^bold>.NID_s1_stm0 \<rightarrow> Skip)\<^bold>; (SSTOP \<triangle>  (exit_stm0 \<rightarrow> Skip))  \<^bold>; ( SSTOP \<triangle>((exited_stm0 \<rightarrow> Skip)\<^bold>; (enter_s0_stm0 \<rightarrow> Skip)))))\<^bold>; Trans_ex1'''\<cdot>NID_s0_stm0)
    \<box>
	  ((share \<rightarrow> Skip)\<^bold>; Trans_ex1'''\<cdot>n)
    \<box> 
    ((interrupt_stm0 \<rightarrow> (SSTOP \<triangle> (exit_stm0 \<rightarrow> Skip)))\<^bold>; (SSTOP \<triangle> (exited_stm0 \<rightarrow>   (terminate \<rightarrow> Skip))))
...
(*this method below uses lemma [simp]: "(SSTOP \<triangle> Q) \<^bold>; R = SSTOP \<triangle> (Q \<^bold>; R)" which is no longer valid.
...
lemma "non_terminating P \<Longrightarrow> P \<^bold>; Q = P"
